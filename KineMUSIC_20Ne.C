...
//
// Usage: root -l MUSIC_Kinematics.C
//
...
/////////////////////////////////////////////////////////////////////////////
{
  ...
  // Beam
  MUSIC->SetBeamParticle("beam", mb, Zb, Kb);
  ...
  MUSIC->SetLightParticle(light, ml, Zl);
  MUSIC->SetEnergyLossFile(light, SRIMFile[2]);
  MUSIC->SetHeavyParticle(heavy, mh, Zh);
  ...
  if(proton==1)
    MUSIC->WriteTraces(Form("TracesR%d_Stp%d_p.root",Reaction,Strip));
  else if(alpha==1)  
    MUSIC->WriteTraces(Form("TracesR%d_Stp%d_a.root",Reaction,Strip));
  else if(neutron==1)MUSIC->WriteTraces(Form("TracesR%d_Stp%d_n.root",Reaction,Strip));
