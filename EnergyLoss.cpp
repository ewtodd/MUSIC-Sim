...
{
  float CurrentE=0, DeltaE=0, Dist=FinalDist/NSteps;
  BraggCurve = new TGraph*[NCurves];
  for (int c=0; c<NCurves; c++) {
    BraggCurve[c] = new TGraph();
    CurrentE = InitE[c];
    //cout << c << " IE=" << CurrentE  << " " << Dist << " " << GetFinalEnergy(CurrentE, Dist, 0.05) << endl;
    for (int s=0; s<NSteps; s++) {
      // Integrate the energy loss in this step (it may be a large step, like in the 
      // detector MUSIC)
      DeltaE = CurrentE - GetFinalEnergy(CurrentE, Dist, 0.05);  // Integrating with 1mm steps.
      if (Energy_in_range) {
	BraggCurve[c]->SetPoint(s, s*Dist, DeltaE);
	//cout << c << " " << s << " " << Dist << " " << DeltaE << " " << CurrentE << endl;
	CurrentE -= DeltaE;
      }
      else
	break;
...
////////////////////////////////////////////////////////////////////////////////////////
double EnergyLoss::GetTimeOfFlight(float InitialEnergy /*MeV*/, float PathLength /*cm*/, float StepSize /*cm*/)
{
  double TOF = 0;
  double Kn = InitialEnergy;
...
bool EnergyLoss::LoadSRIMFile(string FileName)
{
  ...
  if(!Read.is_open()) {
    ...
    // cout << str_counter << endl;  // For debug
    str_counter--;
